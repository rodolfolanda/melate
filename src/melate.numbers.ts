...
function isUniqueSet(numbers: number[], existingSets: number[][], threshold: number = 0): boolean {
...
function generateRandomNumbers(config: { min: number, max: number, count: number }, existingSets: number[][], exclude: number[], threshold: number = 0): number[] {
